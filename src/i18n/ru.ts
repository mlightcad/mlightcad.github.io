...
const parserRu: ParserCopy = {
  ...
  pricingTitle: 'Цены',
  pricingRows: [
    { item: 'Бессрочная лицензия (единовременное пожертвование)', amount: '$3,000 USD', priceKey: 'perpetual' },
    { item: 'Пакеты обновлений — первый год', amount: 'Включено' },
    { item: 'Пакеты обновлений — после первого года', amount: '$1,500 / год', priceKey: 'annual' },
  ],
  pricingNote:
...
export const ru: Dictionary = {
  ...
  tryDrawing: {
    ...
    body: 'Перетащите DWG или DXF сюда. Разбор и отрисовка идут в этой вкладке, файл не загружается на сервер.',
    open: 'Открыть файл',
    trust: 'Без загрузки · без сервера',
    privacyTitle: 'Чертёж не покидает этот браузер',
    privacyBody: 'Файлы не загружаются и не хранятся на наших серверах. Разбор и отрисовка идут в этой вкладке.',
    verifySummary: 'Как в DevTools проверить, что файл не загружается на сервер',
    verifyBody:
      'Откройте инструменты разработчика → Network, пока пользуетесь этим демо. Сам DWG-файл не передаётся на внешний сервер для обработки.',
  ...
  footer: {
    tagline: 'CAD-инфраструктура с открытым исходным кодом для веба.',
